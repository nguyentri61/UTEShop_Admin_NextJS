...
const variantApi = {
  create: (body: CreateProductVariantDtoType) =>
    http.post<ApiResponseType<ProductVariantDetailResponseDtoType>>(
      "/product-variants",
      body
    ),

  list: () =>
    http.get<ApiResponseType<ProductVariantListResponseDtoType[]>>(
      "/product-variants"
    ),
  ...
  sizes: (productId: string) =>
    http.get<ApiResponseType<string[]>>(
      `/product-variants/product/${productId}/sizes`
    ),
  ...
  findByAttributes: (productId: string, size?: string, color?: string) =>
    http.get<ApiResponseType<ProductVariantDetailResponseDtoType>>(
  ...
  update: (id: string, body: UpdateProductVariantDtoType) =>
    http.put<ApiResponseType<ProductVariantDetailResponseDtoType>>(
      `/product-variants/${id}`,
      body
    ),

  remove: (id: string) =>
    http.delete<ApiResponseType<null>>(`/product-variants/${id}`, undefined),
